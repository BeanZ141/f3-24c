...
import sendTicketEmail from './sendEmail.js';
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('ticketForm').addEventListener('submit', async function(event) {
        ...
        try {
            ...
            if (ticketError || !ticketData) throw new Error('No available ticket IDs.');

            // Mark the ticket ID as registered
            const { error: updateError } = await supabase
                .from('ticket_ids')
                .update({ registered: true })
                .eq('tickets', ticketData.tickets);

            if (updateError) throw updateError;

            await sendTicketEmail(email, ticketData.tickets);
